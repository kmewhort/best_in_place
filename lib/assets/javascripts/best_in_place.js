...
BestInPlaceEditor.prototype = {
  ...
  activate : function() {
    ...
    this.element.trigger(jQuery.Event("best_in_place:before_activate"), this);
    this.activateForm();
  ...
  update : function() {
    ...
    if (this.formType == "select") {
      this.previousCollectionValue = this.collectionValue;
        ...
        if(!$.isArray(value)){
          if (value == v[0])
            new_text.push(v[1]);
        	 ...
        	 jQuery.each(value, function(j, jv){
            if (jv == v[0])
              new_text.push(v[1]);       	 
        	 });
        }
      ...
      if(new_text.length == 0)
        ...
        editor.element.html(new_text[0]);
      else{
        comma_separated_elts = new_text.slice(0,new_text.length-2).join(', '); 
        if(comma_separated_elts != "")
          comma_separated_elts += ', ';  
        editor.element.html(comma_separated_elts + new_text.slice(-2).join(', or '));
      }
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function(){
      ...
      self.multiselect       = self.multiselect       || $parent.attr("data-multiselect");
    });
    ...
    self.multiselect       = self.element.attr("data-multiselect")         || self.multiselect;
    ...
    {
      if(self.multiselect){
        // collection values may be empty array
        if(typeof(self.collection) !== 'undefined')
          self.values = jQuery.parseJSON(self.collection);
        else
          self.values = [];

        // collection value may be an array
        try {
          if(typeof(self.collectionValue) !== 'undefined'){
            parsed = JSON.parse(self.collectionValue);
            self.collectionValue = parsed;
          }
          else
  ...
  ajax : function(options) {
    ...
    this.element.trigger(jQuery.Event("ajax:before"), options);
    
    // KM: if no url specified, just keep changes locally with no request
    if((!options.url || (options.url == 'local'))){
  ...
  loadSuccessCallback : function(data) {
    ...
    // KM - not sure what this is supposed to achieve, but doesn't seem to make sense...:
...
BestInPlaceEditor.forms = {
  "input" : {
    activateForm : function() {
      ...
      else { //KM: add default okButton
        output.append(
          jQuery(document.createElement('input'))
          .attr('type', 'submit')
          .attr('class', 'btn btn-primary btn-bip-save')
          .attr('value', 'Save')
        );
      }
      ...
      else { //KM: add default cancelButton
        output.append(
          jQuery(document.createElement('input'))
          .attr('type', 'button')
          .attr('class', 'btn btn-primary btn-bip-cancel')
          .attr('value', 'Cancel')
        );
      }
  ...
  "select" : {
    activateForm : function() {
      ...
      if(this.element.data('multiselect')){
          select_elt.attr('multiple','multiple');
      }
      ...
      jQuery.each(this.values, function (index, value) {
        var option_elt = jQuery(document.createElement('option'))
                         ...
                         .val(value[0])
                         .html(value[1]);
        if($.isArray(currentCollectionValue)){
        	  $.each(currentCollectionValue, function(){
             if(value[0] == this) {
      ...
      this.element.trigger(jQuery.Event("best_in_place:before_bind_events"), this);
      
      if(!this.element.data('multiselect'))
        this.element.find("select").bind('change', {editor: this}, BestInPlaceEditor.forms.select.submitHandler);
      this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.select.submitHandler);
      this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.select.cancelHandler);
      if(!this.element.data('multiselect'))
        this.element.find("select").bind('blur', {editor: this}, BestInPlaceEditor.forms.select.cancelHandler);
